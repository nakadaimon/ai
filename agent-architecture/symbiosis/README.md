...
A real project fans out across the four. A pricing project might land active tasks in `prospective/tasks/`, a draft proposal in `semantic/ideas/drafts/`, source research in `semantic/llm-wiki/raw/`, and corrections from past calls in `episodic/lessons/`. Projects don't get folders; memory does. The taxonomy is by memory type, not by project, because that's what makes the layer transferable across new projects without rearrangement.

Project artifacts themselves — code repositories, design files, large source documents, anything with its own folder structure — don't live inside `memento/`. They sit alongside the symbiosis tree in the workspace (e.g., a `projects/` folder). What goes into memento is *raw research* (in `raw/` substrates) and *distilled knowledge* (wiki, skills, ideas). Memento holds the memory of the work, not the work itself; the structure is agnostic about where the project lives.
...
> I'm bootstrapping a symbiosis layer. The structure exists but the files are empty. Ask me what goes in each — one file at a time: who you are (soul, identity), who I am (user), what we owe each other (the contract), how the work runs (operations, commands), what to remember so far (memento/AGENTS.md). Push back when my answers are thin. You draft, I edit. When we're done, we open a fresh session and run the contract.
